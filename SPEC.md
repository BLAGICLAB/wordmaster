...
| 路由 | 页面 | 内容 |
|------|------|------|
| `#/` | 今日 | 等级称号 + XP 进度条 + streak 火焰；当前词书、今日任务（新学 X / 测验 Y）；提供「学习新单词」直跳 #/study、「开始测验」跳 #/quiz 两个入口；未导入词书时引导去设置页 |
| `#/study` | 学习 | 新学卡片（单词+词性+释义+词根拆解+发音按钮）→「我记住了」→ 立即测验一题 → 下一词 |
| `#/quiz` | 测验 | 今日到期复习队列逐题测验 → 即时反馈 → 队列清空后回今日页并触发打卡庆祝；从今日页或 tab 进入 |
| `#/books` | 词库 | 词书列表（名称/词数/掌握进度环）、设为当前词书、删除、进入详情；默写模式入口；导入仅保留"去导入"跳转链接 |
| `#/books/:id` | 词库详情 | 见下 |
...
4. **关于**：应用名 + 版本号（读 `main.js` 的 `APP_VERSION`，格式 `v2.1.0`）、词库信息（词书数/总词数）、数据说明（仅保存本机浏览器）、词根数据来源及 license 说明
...
- **底部 Tab 栏**：4 项（今日/测验/词库/设置），内联 SVG 图标，激活项主色 + 轻微上浮；「设置」即原「我的」页
...
2. 底部 Tab 最终形态为 4 项：今日 / 测验 / 词库 / 设置。学习（新单词学习流 #/study）由今日页「学习新单词」按钮进入，不单独占 tab；M1–M3 阶段设置页未做时，导入按钮可临时放在词库页，M4 再移入设置页。
...
9. **人工冒烟**：`python3 -m http.server 8000` 起服务，浏览器走一遍 导入→学习新单词→测验→词库详情→统计→设置 全流程。
